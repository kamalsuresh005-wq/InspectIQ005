...
export const ReportPreview: React.FC = () => {
  ...
  // Defensive fallback if inspection data is temporarily empty or loading
  if (!currentInspection || !currentInspection.inspectionNumber) {
    return (
      <div className="max-w-md mx-auto p-6 my-12 text-center bg-white border border-[#D9E1E8] rounded-xl shadow-xs space-y-4">
        <Loader2 className="w-8 h-8 animate-spin text-[#0F766E] mx-auto" />
        <h2 className="text-base font-bold text-[#12304A]">Generating inspection report...</h2>
        <p className="text-xs text-[#52616F]">Loading authoritative inspection data and compliance findings...</p>
        <button
          type="button"
          onClick={() => setFlowStep('final_decision')}
          className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-[#12304A] bg-[#F4F7FA] border border-[#D9E1E8] rounded-lg hover:bg-[#E2E8F0]"
        >
          <ArrowLeft className="w-3.5 h-3.5" />
          <span>Return to Final Decision</span>
        </button>
      </div>
    );
  }
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      setReportError(err?.message || 'Unable to generate the inspection report. Please try again.');
    } finally {
  ...
  const handleViewPdf = async () => {
    ...
    setReportError(null);
    try {
      const { url } = await ReportService.generateInspectionPdfBlob(currentInspection);
      setPdfBlobUrl(url);
      window.open(url, '_blank');
    } catch (err: any) {
      console.error('PDF View Generation Error:', err);
      setReportError(err?.message || 'Unable to generate the inspection report PDF for viewing.');
    } finally {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4 pb-12 p-3 sm:p-4">
      ...
      <div className="bg-white border border-[#D9E1E8] rounded-xl p-3 shadow-xs flex items-center justify-between gap-2 print:hidden">
        ...
        <div className="flex items-center gap-1.5 flex-wrap justify-end">
          ...
          <button
            ...
            className="bg-[#F4F7FA] hover:bg-[#D9E1E8] text-[#12304A] font-bold text-xs py-2 px-3 rounded-lg border border-[#D9E1E8] flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
          >
            {isViewingPdf ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Eye className="w-3.5 h-3.5" />}
            <span>{isViewingPdf ? 'Opening PDF...' : 'View Report'}</span>
          </button>
      ...
      {reportError && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-3.5 text-xs text-red-800 flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            onClick={handleDownloadPdf}
            className="bg-red-600 hover:bg-red-700 text-white font-bold text-[11px] px-3 py-1.5 rounded-lg shrink-0 flex items-center gap-1 cursor-pointer"
          >
